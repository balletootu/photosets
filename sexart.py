...
CHAT_ARR = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K',
            'L', 'M', 'N', 'O', 'P', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Z']


def fetch_model(url, name, head_img):
    '''fetch model'''
    model_dir = os.path.join('sexart', 'model')
    helper.mkDir(model_dir)
    helper.mkDir(os.path.join('sexart', 'photo'))
    # 下载头像先
    helper.downloadImg(head_img, os.path.join(
        model_dir, '%s.jpg' % name))
    if os.path.exists(os.path.join('sexart', 'model', '%s.json' % (name))):
        return
    # 然后去抓取详细数据
    model_info = {
        'name': name,
    ...
    for list_group_item in list_group_item_list:
        ...
        # print date
        # 下载照片的封面
        photo_path = os.path.join('sexart', 'photo', '%s_%s' % (date_str, photo_name.replace(
        ...
        helper.writeFile(photo_json_str, os.path.join(
            'sexart', 'photo', '%s_%s' % (date_str, photo_name), '%s_%s.json' % (date_str, photo_name)))
    helper.writeFile(json.dumps(model_info), os.path.join(
        'sexart', 'model', '%s.json' % (name)))
...
def main(chat, enabled=False):
    ...
    is_enabled = enabled
    if chat_index < 26:
        url = 'https://www.sexart.com/models/all/%s' % CHAT_ARR[chat_index]
        pyquery = helper.get(url)
